...
module.exports = class UserService {
    ...
    async save(req, res) {
        try {
            const { user, userType, walletAddress, maturity, email } = req.body;
            const data = {
                user,
                userType,
                walletAddress,
            ...
            res.status(result.code).send({ Usuario: result.data, message: result.message });
        } catch (error) {
    ...
    */
    async findAll(req, res, where, include) {
        try {
            ...
            const list = await paginateService.paginate(User, paramsQuery.page, paramsQuery.limit,
    ...
    async CreateUsers(req, res) {
        ...
        try {
            ...
            if (idKeycloak) {
                ...
                const idRole = await fetch(process.env.KEYCLOAK_ROLE + role, {
                    method: "GET",
                    headers: {
                        Accept: "application/json",
                        Authorization: `Bearer ${tokenByKeycloak}`,
                ...
                await fetch(process.env.KEYCLOAK_USERS + idKeycloak + '/role-mappings/realm', {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${tokenByKeycloak}`,
                        'Accept': 'application/json',
                ...
                const usuarioCreado = User.create({
                    ...
                    res.status(200).json({
                        users: users,
                        message: "Usuario registrado correctamente, verifique su email para que esta cuenta sea valida"
                    })
                }).catch(err => {
                    res.send(err)
                })
    ...
    async createUserBySocial(req, res) {
        ...
        try {
            ...
            const result = await db.sequelize.transaction(async (t) => {
                ...
                // Asignar a role to user
                const resultRoleMapping = await fetch(process.env.KEYCLOAK_USERS + userId + '/role-mappings/realm', {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${tokenByKeycloak}`,
                        'Accept': 'application/json',
                ...
                if (resultRoleMapping.status != 200 && resultRoleMapping.status != 204) {
                    res.status(500).send(errorsGen.gen_internal);
                }
                return resultCreate;
            });
        ...
        try {
            ...
            if (password !== undefined && confirmPassword !== undefined) {
                ...
                if (password === confirmPassword) {
                    ...
                    const resKeycloak = await fetch(process.env.KEYCLOAK_USERS + userId2, {
                        method: 'PUT',
                        headers: {
                            'Authorization': `Bearer ${tokenByKeycloak}`,
                            'Accept': 'application/json',
                ...
                if (userSearch) {
                    ...
                    if (userKey) {
                        ...
                        const link = url + 'auth/reset-password?token=' + resetToken + '&id=' + userId;
                        const content = "Para continuar el proceso de cambio de contraseña, haga clic en el siguiente enlace: <br> " + link;
                        const send = await emailService.send(userKey.email, 'Password Reset Request', content);
                        console.log('send message', send)
                        const mail1 = (userKey.email).slice(0, 2);
                        const mail2 = (userKey.email).split('@')[1];
                        const mail = mail1 + '*****@' + mail2;
                        return res.status(200).json({
                            message: "Se ha enviado un link para restablecer su contraseña al correo electrónico:",
                            mail: mail,
                        });
                    } else {
